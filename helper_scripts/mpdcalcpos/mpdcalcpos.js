// Caption: LDraw position calculator
...
//	WScript.Echo(inputCodeString);
	...
	arrayTwo.push({fileName:iFile,matrix:FinalRound(iCoos)});
	WalkTree(iSubs,iCoos);
	modelCount += 1;
	objectCount += 1;
...
function WalkTree(inSubFiles, inCoos)
{
	for (var k in inSubFiles)
	{
		...
		{
			if (outMode == 1)
				arrayTwo.push({fileName:kFile,matrix:FinalRound(outCoos)});
			partCount += 1;
		}
		else
		{
